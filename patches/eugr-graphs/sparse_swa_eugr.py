...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # PATCH(gb10-fi614): pad the non-causal DSpark index width up to the
        # nearest width instantiated in flashinfer 0.6.14's decode-dsv4 kernel
        # table ({128, 512, 1024}); off-table widths (e.g. 256 for
        # window=128,k<=128) fall through to the prefill orchestrator, which
        # rejects num_tokens <= 64. Padding is masked by decode_swa_lens.
        _raw_width = cdiv(self.window_size + self.num_speculative_tokens, 128) * 128
        for _w in (128, 512, 1024):
            if _raw_width <= _w:
                _raw_width = _w
                break
        self.noncausal_index_width = _raw_width if self.is_dspark else 0
