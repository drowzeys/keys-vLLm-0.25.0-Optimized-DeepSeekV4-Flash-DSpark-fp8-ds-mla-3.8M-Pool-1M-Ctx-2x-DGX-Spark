...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # PATCH(gb10-dspark-fast): static (empty) attention-metadata dict for
        # the draft forward when VLLM_DSPARK_FAST_DRAFT=1. With the ring-based
        # forward_mqa and the throwaway-block query insert both rebound on the
        # draft layers, nothing in the draft forward reads
        # DeepseekSparseSWAMetadata any more, so the per-step
        # _build_draft_attn_metadata (sparse_swa builder: decode/prefill
        # split, non-causal SWA index kernel, per-type tile-scheduler metas —
        # the "prepare_attn" slice of the step profile) is skipped entirely.
        # The forward only needs a non-None dict so the profile-run checks
        # (``attn_metadata is None`` / ``isinstance(..., dict)``) take the
        # real-run branch.
        self._fast_attn_metadata: dict[str, Any] = {}
    ...
    def capture(self, attn_states: dict | None = None) -> None:
        ...
        # PATCH(gb10-dspark-fast): allocate the draft layers' throwaway
        # query-KV blocks eagerly, before any capture. (The rebound insert
        # also lazily allocates on its first eager call — the manager's
        # warmup pass runs eagerly before the capture pass — but doing it
        # here keeps allocation strictly outside all captured regions.)
        if getattr(self.model, "fast_draft", False):
    ...
    # PATCH(gb10-dspark-qatkv): ring-slot preparation for the DSpark bf16
    # context-KV ring (VLLM_DSPARK_DRAFT_KV_BF16=1).
    @torch.inference_mode()
    def _qat_kv_prepare_ring_slots(
        self,
        input_batch: InputBatch,
        num_rejected: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Select the context tokens to store in the draft's bf16 KV ring.

        The ring aliases positions mod window per persistent request row, so —
        unlike the paged cache, where every position has a unique slot — two
        classes of context tokens must be masked at WRITE time (mirroring
        stage-c's ``store_main_kv`` valid_mask):
          * this step's rejected tokens (they would clobber the in-window slot
            of the position exactly ``window`` earlier), and
          * tokens older than ``valid_end - window`` in long prefill chunks
            (so each ring slot is written at most once per pass; positions
            within a request are strictly increasing, rows are distinct).

        Returns ``(keep_idx, ring_slots)``: indices into the flat context
        token dim and their ``row * window + pos % window`` ring slots.
        """
        window = self.model.qat_kv_window
        num_reqs = input_batch.num_reqs
        num_tokens = input_batch.num_tokens
        device = self.device

        qsl = input_batch.query_start_loc[: num_reqs + 1].long()
        counts = qsl[1:] - qsl[:-1]
        ...
        pos = input_batch.positions[:num_tokens].long()
        nrej = num_rejected[:num_reqs].long()
        valid_counts = counts - nrej
        # Position AFTER the last non-rejected token == prefix length the
        # draft attends from (same as the prepare kernel's last_valid_pos + 1).
        last_valid_idx = (qsl[1:] - nrej - 1).clamp_(min=0)
        valid_end_pos = pos[last_valid_idx] + 1

        tok_off = (
            torch.arange(num_tokens, device=device, dtype=torch.long)
            - qsl[:-1][tok_req]
        )
        keep = (tok_off < valid_counts[tok_req]) & (
            pos >= valid_end_pos[tok_req] - window
        )
        keep_idx = keep.nonzero(as_tuple=False).squeeze(-1)
        rows = input_batch.idx_mapping[tok_req[keep_idx]].long()
        ring_slots = rows * window + pos[keep_idx].remainder(window)
        return keep_idx, ring_slots
    ...
    def propose(
        ...
        # PATCH(gb10-dspark-qatkv): when the DSpark draft keeps its context KV
        # in a private bf16 ring (VLLM_DSPARK_DRAFT_KV_BF16=1), build the ring
        # slots for the non-rejected in-window context tokens and refresh the
        # per-pass request-row mapping. Eager only (this whole section already
        # runs outside the captured graph); skipped on dummy runs like the
        # paged write above.
        if getattr(self.model, "qat_kv_bf16", False):
            qat_keep_idx = qat_ring_slots = None
            if not dummy_run:
                qat_keep_idx, qat_ring_slots = self._qat_kv_prepare_ring_slots(
                    input_batch, num_rejected
                )
                self.model.qat_kv_set_rows(input_batch.idx_mapping, num_reqs)
            self.model.precompute_and_store_context_kv(
                self.hidden_states[:num_target_tokens],
                self.context_positions[:num_target_tokens],
                context_slots,
        ...
        # PATCH(gb10-dspark-fast): in fast mode no draft-layer code consumes
        # the sparse_swa metadata (forward_mqa reads the bf16 ring through the
        # Triton kernels; the query insert uses the private throwaway slots),
        # so skip the whole builder and pass a static non-None dict. Captured
        # FULL graphs contain only kernels reading persistent buffers that the
        # eager prologue (prepare_dflash_inputs + precompute ring writes)
        # refreshes each step, so replay needs no builder-state update either.
        if getattr(self.model, "fast_draft", False):
            draft_attn_metadata = self._fast_attn_metadata
        else:
    ...
    if block_idx == 0:
        ...
        if req_idx == num_reqs - 1:
            ...
            for i in range(q_pad_start, max_num_tokens, BLOCK_SIZE):
                ...
                # PATCH(gb10-dspark-cg): pad input_ids rows too — stale anchor ids
                # otherwise feed the Markov embedding gather under CG replay.
                tl.store(out_input_ids_ptr + block, parallel_drafting_token_id, mask=mask)
