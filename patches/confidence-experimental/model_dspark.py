...
# PATCH(gb10-dspark-conf): confidence head, ported from the stage-c
# DeepSeekV4DSparkConfidenceHead. One fp32 linear over
# [pre-norm head hidden | markov embedding of the predecessor token] emitting a
# raw confidence logit per draft position (caller applies sigmoid).
# Checkpoint weight: mtp.<last>.confidence_head.proj.weight
# (shape [1, hidden_size + dspark_markov_rank], bf16 -> cast to fp32 on load).
class DSparkConfidenceHead(nn.Module):
    def __init__(self, hidden_size: int, markov_rank: int, *, prefix: str) -> None:
        super().__init__()
    ...
    def forward(
        ...
        features = torch.cat([hidden_states.float(), markov_embed.float()], dim=-1)
...
class DSparkDeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # PATCH(gb10-dspark-conf): confidence head (checkpoint: mtp.<last>.
        # confidence_head.proj.weight). Built whenever a Markov rank exists;
        # the ForCausalLM wrapper tracks whether its weight actually loaded.
        self.confidence_head: DSparkConfidenceHead | None = None
        if int(getattr(config, "dspark_markov_rank", 0)) > 0:
...
class DSparkDeepseekV4ForCausalLM(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # PATCH(gb10-dspark-conf): set true in load_weights when the checkpoint
        # actually provides confidence_head.proj.weight; the speculator gates
        # confidence scheduling on this flag.
    ...
    # PATCH(gb10-dspark-conf): raw confidence logit per draft position.
    # ``hidden_states`` is the PRE-norm hc_head hidden at the draft position
    # (the same tensor compute_logits norms), ``markov_embed`` is the Markov
    # embedding of that position's predecessor token — both are already live
    # in the speculator's sequential sampling loop. Caller applies sigmoid.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        self._finalize_moe()
        # PATCH(gb10-dspark-conf): confidence scheduling is only enabled when
        # the checkpoint actually shipped the head weight.
        self.has_confidence_head = (
            "model.confidence_head.proj.weight" in loaded_params
        )
        if self.has_confidence_head:
            logger.info_once("DSpark confidence head loaded.")
    ...
    def _remap_dspark_name(self, name: str) -> str | None:
        ...
        stage = int(m.group(1))
        rest = m.group(2)
        # PATCH(gb10-dspark-conf): the confidence head IS wired into inference
        # now (per-position draft-length scheduling); load its weights. If the
        # module was not built (no markov rank), drop them as before.
        ...
        head_prefixes = (
            ...
            "hc_head_scale",
            "markov_head.",
            # PATCH(gb10-dspark-conf): confidence head lives at model level.
            "confidence_head.",
        )
