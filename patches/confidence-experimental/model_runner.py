...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def sample_tokens(
        ...
        if self.num_speculative_steps > 0:
            ...
            # PATCH(gb10-dspark-conf): when the speculator exposes confidence
            # scores (DSpark confidence head), hand them plus the CPU length
            # scheduler to the handler so take_draft_token_ids() returns
            # per-request variable-length draft lists. In synchronous
            # scheduling these flow via Scheduler.update_draft_token_ids ->
            # request.spec_token_ids, so the next step schedules and verifies
            # only the scheduled prefix per request.
            draft_confidences = None
            draft_length_scheduler = None
            if self.speculator is not None and getattr(
                self.speculator, "confidence_scheduling_enabled", False
            ):
                draft_confidences = self.speculator.get_draft_confidences(
                    input_batch.num_reqs
                )
                draft_length_scheduler = self.speculator.schedule_draft_lengths
            self.draft_tokens_handler.set_draft_tokens(
                input_batch,
                self.req_states.draft_tokens[input_batch.idx_mapping],
                draft_confidences,
                draft_length_scheduler,
            )
